...
def get_all_players():
    """Hämtar alla aktuella spelare dynamiskt från LMS."""
    res = lms_json_rpc("", ["players", "0", "10"])
    if res and 'result' in res:
        return res['result'].get('players_loop', [])
    return []
...
def play_url():
    ...
    # --- SPELLISTOR: Shuffle PÅ ---
        ...
        lms_json_rpc(player_mac, ["playlist", "shuffle", 1])
        # Och vi hoppar till en slumpmässig låt direkt i den nyss laddade listan
...
def search_and_play():
    ...
    mac, room_name = get_player_info(request.args.get('room'))
    
    if not mac:
        return jsonify({"error": "Spelare hittades ej"}), 404

    # Här anropar vi spotty-pluginen direkt via LMS JSON-RPC
    # Spotty har ofta en sök-funktion: ["spotty", "search", query]
    # Du kanske behöver experimentera lite med exakt syntax för ditt LMS
    result = lms_json_rpc(mac, ["spotty", "search", query, "limit:10"])
    
    # För enkelhetens skull: ta första träffen och spela upp
    # (Logiken kan byggas ut för att välja slumpmässigt bland topp 5)
    if result and 'result' in result:
        # Pseudo-kod för att trigga uppspelning av första sökresultatet
        track_uri = result['result']['tracks'][0]['uri']
        lms_json_rpc(mac, ["playlist", "clear"])
        lms_json_rpc(mac, ["playlist", "add", track_uri])
        lms_json_rpc(mac, ["play"])
        return jsonify({"status": "ok", "played": query})
        
    return jsonify({"error": "Ingen musik hittades"}), 404
        ...
        if mode == 'play':
            ...
            if loop:
                track = {
                    ...
                    'artist': loop[0].get('artist', ''),
                }
            playing.append({'room': room, 'mac': mac, 'mode': mode, 'track': track})
...
def spotify_artist_top():
    ...
    # Först: sök rätt på artisten för att få dess Spotty-ID
    encoded = urllib.parse.quote(query)
    # item_id:1.0 är sökmenyn, .0 är Artists-kategorin
    res = lms_json_rpc(player_mac, ["spotty", "items", 0, 5, f"item_id:1.0_{encoded}.0"])
    if not res or 'result' not in res or not res['result'].get('loop_loop'):
        return jsonify({"error": "Artist hittades ej"}), 404
    
    artist_id = res['result']['loop_loop'][0]['id']
...
if __name__ == '__main__':
    ...
    print("--- Lyrionbridge v2: Startad (port 5000) ---")
